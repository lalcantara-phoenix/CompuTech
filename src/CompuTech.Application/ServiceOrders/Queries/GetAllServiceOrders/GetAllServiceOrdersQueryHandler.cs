...
public class GetAllServiceOrdersQueryHandler(IServiceOrderRepository serviceOrderRepository)
    : IRequestHandler<GetAllServiceOrdersQuery, IReadOnlyList<ServiceOrderDto>>
{
    public async Task<IReadOnlyList<ServiceOrderDto>> Handle(GetAllServiceOrdersQuery request, CancellationToken ct)
    {
        var orders = await serviceOrderRepository.GetAllAsync(ct);
